...
def create_nuscenes_infos(root_path,
    ...
    train_nusc_infos, val_nusc_infos = _fill_trainval_infos(
        nusc, train_scenes, val_scenes, out_dir, test, max_sweeps=max_sweeps)
    ...
    if test:
        ...
        info_val_path = osp.join(pkl_path,
...
def _fill_trainval_infos(nusc,
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        lidar_img_folder_paths = [out_dir+'/lidar_samples/rih/',out_dir+'/lidar_samples/xz0/',
                                out_dir+'/radar_samples/riv/',out_dir+'/radar_samples/xz0/']
        ...
        for cam in camera_types:
            ...
            for pointsensor_channel in ['RADAR_FRONT', 'RADAR_FRONT_LEFT', 'RADAR_FRONT_RIGHT', 'RADAR_BACK_LEFT', 'RADAR_BACK_RIGHT']:
                ...
                pointsensor_calibrated = nusc.get('calibrated_sensor', pointsensor['calibrated_sensor_token'])
                pcl_path = osp.join(nusc.dataroot, pointsensor['filename'])
                current_sweep = pointsensor
                sweep_paths = []
                for _ in range(10): #Sweeps
                    if current_sweep['prev'] == '':
                        break
                    else:
                        current_sweep = nusc.get('sample_data',current_sweep['prev'])
                    sweep_paths.append(osp.join(nusc.dataroot, current_sweep['filename']))
                info['radar_pcl'][pointsensor_channel] = dict(filename=pcl_path, calibrated_sensor=pointsensor_calibrated,sweep_paths=sweep_paths)
    ...
    for info in mmcv.track_iter_progress(nusc_infos):
        for cam in camera_types:
            ...
            lidar_img_info.update(
                    ...
                    token=info['token'])
            lidar_img_info['rih']['file_name'] = lidar_img_info['rih']['file_name'].split(root_path+'/')[-1]
            lidar_img_info['xz0']['file_name'] =lidar_img_info['xz0']['file_name'].split(root_path+'/')[-1]
            ...
            radar_img_info.update(
                    ...
                    token=info['token'])
            radar_img_info['riv']['file_name'] = radar_img_info['riv']['file_name'].split(root_path+'/')[-1]
            radar_img_info['xz0']['file_name'] =radar_img_info['xz0']['file_name'].split(root_path+'/')[-1]
