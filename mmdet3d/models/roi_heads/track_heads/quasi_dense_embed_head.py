...
from mmdet3d.core import cal_similarity
...
class QuasiDenseEmbedHead(nn.Module):
    ...
    def get_track_targets_gt_only(self, gt_match_indices, key_sample_ids, ref_sample_ids, batch_size):
        track_targets = []
        track_weights = []
        ...
        for _gt_match_indices, num_key_roi, num_ref_roi in zip(gt_match_indices,
                                                       ...
                                                       num_ref_rois):
            targets = _gt_match_indices.new_zeros(
                (num_key_roi, num_ref_roi),
                dtype=torch.int, device=_gt_match_indices.device)
            _match_indices = _gt_match_indices
            pos2pos = (_match_indices.view(
                -1, 1) == torch.arange(num_ref_roi, device=_match_indices.device).view(1, -1)).int()
            targets[:, :pos2pos.size(1)] = pos2pos
            weights = (targets.sum(dim=1) > 0).float()
            track_targets.append(targets)
            track_weights.append(weights)
    ...
    def match_gt_only(self, key_embeds, ref_embeds, key_sample_ids, ref_sample_ids, batch_size):
        """Matches embeddings, but only for ground truth boxes.
        """
        num_key_rois = tuple(np.bincount(key_sample_ids.astype(np.uint8), minlength=batch_size).astype(int))
        num_ref_rois = tuple(np.bincount(ref_sample_ids.astype(np.uint8), minlength=batch_size).astype(int))
        # assert len(num_key_rois) == len(num_ref_rois), "Number of key and ref rois should be the same, maybe one sample does not have gt boxes in the last frame?"
