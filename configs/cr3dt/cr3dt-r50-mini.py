...
# TODO: Adapt the fused size according to the fusion idea
#       E.g. if we leave out the bev_compressor, we need to adapt the fused size (will grow)
numC_Trans_Fused = numC_Trans + 18 # TODO: Don't forget to change this size if you remove the bev_compressor or add the residual connection!
...
model = dict(
    ...
    radar_voxel_encoder=dict(type='RadarPillarFE', grid_config=radar_grid_config, transpose=False, rot=False, radar_feat_dim=18), # Our custom PillarFE which also rasterizes the points
    # If commented out, the features will simply be concatenated - if you want to use it, you need to change the radar_feat_dim to the one you use
    # bev_compressor=dict(img_feat_dim=numC_Trans, pts_feat_dim=pts_z_num_cells, radar_feat_dim=16 * radar_z_num_cells, img_grid_height=img_z_num_cells), 
    ...
    pts_bbox_head=dict(
        type='CenterHead',
        in_channels=256 + 18, # TODO: Add 18 radar channels if residual connection is used
        tasks=[
